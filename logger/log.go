...
func initPoolLogger() *log.Logger {
	logFile, _ := os.Create("pool.log")
	loger := log.New(logFile, "[Pool]", log.LstdFlags|log.Lshortfile|log.LUTC) // 将文件设置为loger作为输出
	return loger
}
